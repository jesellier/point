...
class  LaplaceApproximation( ModelAdapter) :
    ...
    def __init__(self, model, X = None):
        ...
        self.default_opt_method = opt_method.L_BFGS
        #self.default_opt_method =  opt_method.NEWTON_CG
    ...
    def predict_f(self, X_new):
        ...
        self._L = tf.linalg.cholesky(- self.log_posterior_H())
    ...
    def predictive_log_likelihood(self, X_new):
        ...
        integral_term = tf.reduce_sum(tf.eye(self.n_components, dtype=default_float()) * V) 
    ...
    def log_posterior(self, get_grad = False):
        ...
        if get_grad is True :
            ...
            grad = - (2 * self.M + tf.eye(self.n_components,  dtype= default_float())) @ self.latent + 2 * tf.expand_dims( tf.reduce_sum( self.feature / f, 0), 1)
    ...
    def log_posterior_H(self):
        ...
        M =  self.M + 0.5 * tf.eye(self.n_components,  dtype= default_float())
    ...
    def log_marginal_likelihood(self, get_grad = False, adjust_der = True, get_implicit = False):
        ...
        out = self.log_posterior()  -  0.5 * tf.linalg.logdet(-self.log_posterior_H())
        return out
    ...
    def __get_implicit_der(self):
        ...
        if self.hasDrift :
            ...
            cache = tf.ones((self.n_components,1),  dtype=default_float())
