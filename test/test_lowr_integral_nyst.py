...
from point.helper import method, get_process, get_lrgp
...
class TestNystrom(unittest.TestCase):
    ...
    def test_singularity(self):
        lrgp = get_lrgp(
            length_scale = self.length_scale, variance = self.variance, 
    ...
    def test_full_integral(self):
        ...
        Psi = tf_calc_Psi_matrix_SqExp(lrgp._x, variance, length_scale,  domain = lrgp.space.bounds )
        ...
        #recalculated2  sum_{i,j} prod * phi_i^T Psi phi_j
        out_recalc2 = 0
        u = tf.linalg.diag(1/tf.math.sqrt(lrgp._lambda)) @ lrgp._latent
        for i in range(lrgp.n_components):
            for j in range(lrgp.n_components):
                quad = tf.expand_dims(lrgp._U[:, i], 0) @ Psi @ tf.expand_dims(lrgp._U[:, j], 1)
                out_recalc2 += u[i] * u[j] * quad
  
        out_recalc2 = out_recalc2.numpy()[0][0]
        if self.verbose : print("recalc2:= %f" % (out_recalc2))


        #implemented
        out_impl = lrgp.integral().numpy()
        if self.verbose : print("impl:= %f" % (out_impl))
        
        #num integration
        def func1(x,y):
            points = tf.constant([x,y], dtype=float_type)
            out = lrgp.func(points)
            return out**2

        out_num = integrate.dblquad( lambda x,y: func1(x,y), bounds[0], bounds[1],bounds[0], bounds[1])
        out_num = out_num[0]
        if self.verbose : print("num:= %f" % (out_num))
    ...
    def test_full_integral_with_beta(self):
        "test the implementation of int (f + beta)^2"
        
        variance = self.variance
        length_scale = self.length_scale
        beta0 = 0.5

        lrgp = get_lrgp(beta0 = beta0, length_scale = length_scale, variance = variance, 
                                   space = self.space, method = method.NYST, n_components = 250, random_state = self.rng)
        
        bounds = lrgp.space.bounds1D
        lrgp._lambda = tf.ones(shape = [lrgp.n_components], dtype=float_type)  #Lambda set to 1 (otherwise cause numerical problems in dblquad)
